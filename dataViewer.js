...
DataViewer.defaultsBridgeDataTable = {
        ...
        success: function(response) {
          ...
          if ($(configObj.response).length > 0 || !configObj.successEmpty) {
            ...
            if (configObj.successEmpty) {
              ...
              //configObj = configObj.renderer.type(destination, configObj);  // Should this be enabled to allow an empty table to be created?
            }
          }
...
DataViewer.Renderers = {
  ...
  DataTables: function(destination, configObj) {
    ...
    configObj = $.extend({}, DataViewer.defaultsBridgeDataTable, configObj);
    ...
    if (configObj.responsive) {
      ...
      // Apply sorting to 2nd column so the control column isn't sorted
      if (!configObj.renderer.options.order) {
        configObj.renderer.options.order = [[1, 'asc']];
      }
    }
    ...
    if (
      ...
      tableContainer.off('click').on('click', 'td', function(event) {
  ...
  UnorderedList: function(destination, configObj) {
    // Entend defaults into the configuration
    configObj = $.extend({}, Dataviewer.defaultsBridgeList, configObj);
    // Merge Render options into Config Obj
    configObj = $.extend(true, {}, configObj, configObj.renderer.options);
    // Ensure the response is an Array
...
/*
  **
  JQuery cannot be extended when using React.  This function would need to be re-written if needed.  Lists are not currently used.
  **
