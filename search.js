...
(function($){
    ...
    var defaultsApiDataTable  = {
        execute: performApiRequestDataTable,
        resultsContainer : '<table cellspacing="0" class="table dataTable table-striped table-bordered nowrap dtr-inline">',
        apiConfig:{
        },
        // Properties specific to DataTables
    ...
    function performApiRequestDataTable(){
        var configObj = this;
        if(configObj.before){configObj.before();};
        convertDataToColumns(configObj);
        if(typeof configObj.apiConfig.url == "function"){
            configObj.apiConfig.url = configObj.apiConfig.url();
        }
        $.ajax({
            url: configObj.apiConfig.url, 
            method: 'get',
            contentType: 'application/json',
            success: function(response) {
                configObj.dataArray = [];
                configObj.response=response.submissions;
                if($(configObj.response).size() > 0 || !configObj.successEmpty){
                    // Execute success callback
                    if(configObj.success){configObj.success();}
                    // Only one record returned
